...
namespace TrackerLibrary
{
    public static class TournamentLogic
    {
        ...
        private static void CompleteTournament(TournamentModel model)
        {
            ...
            TeamModel winners = model.Rounds.Last().First().Winner;//get the last rounds
            ...
            if(winnerPrize > 0)
            {
                body.AppendLine($"<p>{winners.TeamName} will receice ${winnerPrize}</p>");
            }

            if (runnerUpPrize > 0)
            {
                body.AppendLine($"<p>{runnerUp.TeamName} will receice ${runnerUpPrize}</p>");
            }
